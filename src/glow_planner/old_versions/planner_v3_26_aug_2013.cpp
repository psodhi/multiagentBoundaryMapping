...
//			}
	...
	void imageCb(const sensor_msgs::ImageConstPtr& msg)
	{
		...
		for(int i=0;i<NO_BOTS;i++)
		{
			...
			float goal_heading=atan2f((resultant.y-bot_pose[i].y),(resultant.x-bot_pose[i].x));
			float error_angle=goal_heading-bot_pose[i].theta;
			drawArrow(result, Point2f(bot_pose[i].x, bot_pose[i].y), headingPointCalc(Point2f(bot_pose[i].x, bot_pose[i].y), goal_heading), bot_color[i]);
			...
			else
			{
				...
				drawArrow(bot_plan[i], Point2f(bot_pose[i].x, bot_pose[i].y), headingPointCalc(Point2f(bot_pose[i].x, bot_pose[i].y), goal_heading), Scalar(0,255,0));
