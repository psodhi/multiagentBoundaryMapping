/*
Glowworm Swarm optimisation with repelling radius
Repelling radius heading calculated if bot is too close to another robot. This caused both robots that that were too closed to each other to head tangential to the repelling radius (in other words, in the opposite direction)
*/
...
class ImageConverter
{
		...
		: it_(nh_)
	{
		...
		cv::namedWindow("gso_result", CV_WINDOW_AUTOSIZE);
			...
			if(light_initialised)
			{
				for(int i=0;i<NO_BOTS;i++)	//Reset new light intensities flag
				{
					if(!bot_goal[i])
						...
						{
							drawArrow(path, old_position[i], Point2f(bot_pose[i].x, bot_pose[i].y), Scalar(0,0,0));
							imshow("Path",path);
						}
					#endif
				}
