...
class ImageConverter
{
	...
	int light_intensity[NO_BOTS], front_data[NO_BOTS], light_intensity_raw[NO_BOTS];
...
// Publishing front move acknowledgement on the same topic as reading light_intensity
	...
	void bot0IntensityCb(const std_msgs::UInt16ConstPtr& msg) // scale up
	{
		...
		light_intensity_raw[0] = light_intensity[0];
	...
	void bot1IntensityCb(const std_msgs::UInt16ConstPtr& msg)
	{
		...
		light_intensity_raw[1] = light_intensity[1];
	...
	void bot2IntensityCb(const std_msgs::UInt16ConstPtr& msg)
	{
		...
		light_intensity_raw[2] = light_intensity[2];
	...
	void bot3IntensityCb(const std_msgs::UInt16ConstPtr& msg)
	{
		...
		light_intensity_raw[3] = light_intensity[3];
	...
	void bot4IntensityCb(const std_msgs::UInt16ConstPtr& msg)
	{
		...
		light_intensity_raw[4] = light_intensity[4];
	...
	void imageCb(const sensor_msgs::ImageConstPtr& msg)
	{
		...
		*/
		...
		{
			string filename_dynamic = "/home/paloma/Pictures/gso_videos/gso_result_dynamic.avi";
			string filename_static = "/home/paloma/Pictures/gso_videos/gso_result_static.avi";
			string filename_path = "/home/paloma/Pictures/gso_videos/gso_result_path.avi";
			...
			filename_bot_plan[0] = "/home/paloma/Pictures/gso_videos/gso_bot0_plan.avi";
			filename_bot_plan[1] = "/home/paloma/Pictures/gso_videos/gso_bot1_plan.avi";
			filename_bot_plan[2] = "/home/paloma/Pictures/gso_videos/gso_bot2_plan.avi";
			filename_bot_plan[3] = "/home/paloma/Pictures/gso_videos/gso_bot3_plan.avi";
			filename_bot_plan[4] = "/home/paloma/Pictures/gso_videos/gso_bot4_plan.avi";
		...
		{	
			...
			sprintf(label,"I: %d",light_intensity_raw[i]);
...
//					if(!bot_goal[i])
			...
			if(light_initialised)
			{
				...
				for(int i=0;i<NO_BOTS;i++)
				{
					...
					{
						if(light_intensity[i] == light_intensity[j]) // fix
						{
...
//				float error_angle=goal_heading[i]-bot_pose[i].theta;
		...
		if((state == ROTATE) && (callback_count == 4) && (frame_count > 4))
		{
			...
			for(int i = 0;i < NO_BOTS; i++)
			{
				...
				if((abs(error_angle_degrees) > 5) && (heading_correct_counter[i] < 10))
				{
					//Correct heading;
