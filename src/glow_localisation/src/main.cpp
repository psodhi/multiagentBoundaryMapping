...
class ImageConverter
{
	...
	/* Logitech Camera Settings */
			...
			min_sat_bot[1]=85;
			max_sat_bot[1]=211;
			min_val_bot[1]=200;//133=night;
			max_val_bot[1]=255;
			
	/* Samsung S3 Camera Settings 
	...
	void drawArrow(Mat image, Point2f p, Point2f q, Scalar color, int arrowMagnitude = 12, int thickness=1.5) 
	{
...
//						cout << i << " " << calculateDistance(location_blob[i].location,location_blob[rear_match_index].location)<< endl;
	...
	//				cout<<i<<" "<<(bot_pose[i].theta*57.29f)<<endl;
					...
					drawArrow(result, location_blob[rear_match_index].location, location_blob[i].location, Scalar(0,0,0));
//					char heading_degrees[30];
//					sprintf(heading_degrees, "%f", bot[i].heading*360/(2*PI));
//					insertText(result, location_blob[rear_match_index].location, heading_degrees, Scalar(0,0,0));
...
//		bot_pose_pub[i].publish(bot_pose[i]);
	...
	void imageCb(const sensor_msgs::ImageConstPtr& msg)
	{
		...
		{
			string filename_localization = "/home/paloma/Pictures/gso_videos/localization_result.avi";
			string filename_source = "/home/paloma/Pictures/gso_videos/localization_source.avi";
